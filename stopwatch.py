...
class stopwatch:
	def __init__(self, master):
		...
		self.timeLabel = tk.Label(master, textvariable=self.runningTime, font=('Times New Roman', 20))
		self.timeLabel.grid(row=0, column=1)
		self.timeLabel.pack()

		self.toggleButton = tk.Button(master, textvariable=self.toggleString, command=self.toggle, relief='raised')
	...
	def toggle(self):
		with open('history.txt', 'a') as historyTXT:
			if self.toggleString.get() == 'Start':
				...
				historyTXT.write(time.strftime('%Y:%m:%d\t%H:%M:%S') + '\tPaused at: ' + self.runningTime.get() + '\n')
				self.pause()
			elif self.toggleString.get() == 'Unpause':
				historyTXT.write(time.strftime('%Y:%m:%d\t%H:%M:%S') + '\tUnpaused at: '+ self.runningTime.get() + '\n')
				self.unpause()
